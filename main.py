...
intenciones = {
    ...
    "si por favor": "Perfecto, ¿Cuál es tu nombre?",
    "tienes credito": "Perfecto, ¿Cuál es tu nombre?",
    "Aprubas credito ": "Perfecto, ¿Cuál es tu nombre?",
    "tienes finaciamiento": "Perfecto, ¿Cuál es tu nombre?",
    "finacias": "Perfecto, ¿Cuál es tu nombre?",
...
# 🔍 Fuzzy Matching
def encontrar_intencion(texto_usuario: str, umbral=90):
...
def encontrar_similar(texto, opciones, umbral=90):
...
async def responder(mensaje: Mensaje):
    ...
    if color_encontrado and tipo_encontrado:
        ...
        return {
            "respuesta": f"Agregado: {tipo_encontrado} {color_encontrado} cuando quieras ver el catálogo, puedes ir a la sección de carros. ¿Quiéres un finaciamiento de tu carro seleccionado",
    ...
    if "me llamo" in texto:
        ...
        return {
            "respuesta": f"Perfecto {nombre}, ¿cuántos años tienes?",
    ...
    if "tengo" in texto and "años" in texto:
        ...
        nombre_actual = datos_usuario.get("nombre", "")
        return {
            "respuesta": f"Ok {nombre_actual}, tue edad es {edad} años, ¿cuál es tu estado civil?",
    ...
    if "gano" in texto or "ingreso" in texto:
        ...
        return {
            "respuesta": f"Tus ingresos registrados son {ingresos}, ¿Cuanto tiempo llevas laborando?",
    ...
    if "independiente" in texto:
        ...
        nombre_actual = datos_usuario.get("nombre", "")
        return {
            "respuesta": f"vale {nombre_actual} tu ocupación es independiente, ¿cuánto tiempo llevas laborando?",
            "accion": "REGISTRAR_OCUPACION",
        }

    if "soltero" in texto or "soltera" in texto:
        datos_usuario["ocupacion"] = "soltero"
        nombre_actual = datos_usuario.get("nombre", "")
        return {
            "respuesta": f"vale {nombre_actual} tu ocupación es soltero/ra, ¿cuánto tiempo llevas laborando?",
            "accion": "REGISTRAR_OCUPACION",
        }
    ...
    if "pensionado" in texto:
        ...
        nombre_actual = datos_usuario.get("nombre", "")
        return {
            "respuesta": f"vale {nombre_actual} tu ocupación es pensionado, ¿cuánto tiempo llevas laborando?",
            "accion": "REGISTRAR_OCUPACION",
        }
    ...
    if "meses" in texto:
        ...
        if (
            ...
            ultimo_usuario = historial_aprobaciones[
                -1
            ]  # Guarda referencia al último registro
            ...
            return {
                ...
                "accion": accion_final,
                "usuario": ultimo_usuario,  # Enviar también el último usuario registrado
            }

        # Si aún no se puede verificar, solo registrar el tiempo
        return {
            "respuesta": f"Tiempo registrado  {tiempo} meses de labor, ¿Cuanto ganas mensualmente?",
